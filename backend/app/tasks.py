...
if Celery is not None:
    ...
    celery_app = Celery("DFIR-Lab_tasks", broker=broker_url, backend=broker_url)
else:
...
def generate_pdf_report(evidence_id: str, output_path: str):
    ...
    try:
        ...
        # Header text
        story.append(Paragraph("DFIR-Lab Digital Forensic Report", title_style))
        ...
        def _add_page_frame(canvas_obj, doc_obj):
            ...
            # Footer
            footer_text = f"DFIR-Lab â€” Generated for {ev.organization_name or 'Unknown Organization'} | {datetime.datetime.utcnow().date.year}"
    ...
    def generate_audit_xlsx(rows=None):
        ...
        ws.cell(row=title_row, column=1, value="DFIR-Lab â€” Audit Logs").font = Font(size=14, bold=True)
